...
# medicarebot_cb is the name of the database
# medicarebot is the name of the table which is having 4 fied id, query, addressed, query_type
def insert_to_db(qt, q, a):
    sql = "insert into `medicarebot` (`query`, `addressed`, `query_type`) values ('{0}', '{1}', '{2}')".format(
        q, a, qt)
    print(sql)
    con = create_engine("mysql+pymysql://root:@127.0.0.1:3307/medicare_cb")
    con.execute(sql)
